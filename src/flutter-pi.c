...
void           pageflip_handler(int fd, unsigned int frame, unsigned int sec, unsigned int usec,
                                void *userdata)
{
...
bool init_display(void)
{
    ...
    if (!drm.has_device) {
        ...
        for (i = 0; i < num_devices; i++) {
            ...
            if (resources == NULL) {
                printf("      could not query DRM resources for DRM device candidate at \"%s\":",
                       device->nodes[DRM_NODE_PRIMARY]);
...
void  init_io(void)
{
    ...
    for (int i = 0; i < input_devices_glob.gl_pathc; i++) {
        ...
        printf("      %s, connected via %s. vendor: 0x%04X, product: 0x%04X, version: 0x%04X\n", dev.name,
               INPUT_BUSTYPE_FRIENDLY_NAME(dev.input_id.bustype), dev.input_id.vendor, dev.input_id.product,
               dev.input_id.version);
...
close_continue:
    ...
    if (!ok) fprintf(stderr,
                         "error while sending initial mousepointer / multitouch slot information to flutter\n");
}
    ...
    while (n_ready_fds > 0) {
        ...
        for (int i = 0; i < n_linuxevents; i++) {
            ...
            if (e->type == EV_REL) {
                ...
                } else if (e->code == ABS_MT_POSITION_X || e->code == ABS_X || e->code == ABS_MT_POSITION_Y
                           || e->code == ABS_Y) {
                    double relx = 0, rely = 0;

                    if (e->code == ABS_MT_POSITION_X || (e->code == ABS_X && device->n_mtslots == 1)) {
                        double newx = (e->value - device->xinfo.minimum) * width / (double) (device->xinfo.maximum -
                                                                                             device->xinfo.minimum);
                        ...
                        double newy = (e->value - device->yinfo.minimum) * height / (double) (device->yinfo.maximum -
                                                                                              device->yinfo.minimum);
